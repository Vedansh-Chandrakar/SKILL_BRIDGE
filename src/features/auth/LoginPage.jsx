...
export default function LoginPage() {
  ...
  const handleLogin = async (e) => {
    ...
    else if (loginEmail.includes('campus') || loginEmail.includes('authority')) role = 'campus';
    login(role);
    navigate({ [ROLES.ADMIN]: '/admin', [ROLES.CAMPUS]: '/campus', [ROLES.STUDENT]: '/student' }[role] ?? '/student');
  };
  ...
  const handleRegister = async (e) => {
    ...
    login(
      regForm.role === 'campus' ? 'campus' : 'student',
      regForm.role === 'student' ? regForm.studentMode : undefined,
    );
    navigate(regForm.role === 'campus' ? '/campus' : '/student');
  };
      ...
      <div className="relative w-full max-w-[1000px] rounded-3xl overflow-hidden shadow-2xl shadow-indigo-200/50 grid grid-cols-1 lg:grid-cols-2 bg-white">
        ...
        <div className="relative min-h-[620px] flex items-center justify-center px-6 py-10 sm:px-10 lg:px-12 overflow-hidden order-first lg:order-last">
          ...
          <div className={`w-full max-w-sm transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] ${!isSignUp ? formIn : formOut}`}>
            ...
            <div className="mt-5 rounded-2xl bg-indigo-50/80 border border-indigo-100 p-3"><p className="text-center text-xs text-indigo-600"><strong>Demo:</strong> Use <em>admin</em>, <em>campus</em>, or any email for student</p></div>
          </div>
